...
public enum EnumOrgType {
	GROUP("1","集团"),//指拜博集团，机构树中的根节点
	REGION("2","大区"),//“集团”本身作为与大区平级的存在，父节点为拜博集团
	BUSINESS_UNIT("3","事业部"),
	SUB_BUSINESS_UNIT("4","子事业部"),
	DEPARTMENT("5","部门"),
	HOSPITAL("6","医院/门诊");
	...
	private EnumOrgType(String code, String msg) {
		this.code = code;
		this.msg = msg;
		this.display = true;
	}
	 ...
	 * @return 枚举对象
	 */
	public static EnumOrgType getEnumByCode(String code) {
		if (code == null) {
			return null;
		}
		for (EnumOrgType type : values()) {
	...
	public static Map<String, String> toMixMap() {
		...
		for (EnumOrgType type : values()) {
			if (type.isDisplay()) {
				enumDataMap.put(type.getCode(),
						type.getCode() + "-" + type.getMsg());
			}
		}
		return enumDataMap;
	}
